...
const editComment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await deleteOldFiles(filesDir, oldImages, editedImages);
    await deleteOldFiles(filesDir, oldVideos, editedVideos);
...
const deleteComment = async (
  ...
  try {
    const { id } = req.body;
    const comment = await Comment.findOne({ where: { uuid: id } });
