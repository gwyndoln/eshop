...
const editCommentParser = (req: Request, res: Response, next: NextFunction) => {
	...
	let filesCounter = 0;
	const editedImagesArray: string[] | [] = [];
	const editedVideosArray: string[] | [] = [];
	...
	form.onPart = (part) => {
		...
		if (field === 'text' || field === 'commentId') {
			return form._handlePart(part);
		}

		if (field === 'editedImages' || field === 'editedVideos') {
			part.on('data', (buffer) => {
				let editedFilesArray =
					field === 'editedImages' ? editedImagesArray : editedVideosArray;

				editedFilesArray = editedFilesArray.concat(
					buffer.toString().split(';')
				);

				filesCounter += editedFilesArray.length;
			});
			return form._handlePart(part);
		}
...
export default editCommentParser;
